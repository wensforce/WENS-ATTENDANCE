...
const DesignationCreateModal = ({ open, onClose, onSubmit, loading = false, initialValue = "" }) => {
      ...
      <div className="relative w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden bg-surface">
        ...
        <div className="flex items-start justify-between px-6 pt-5 pb-4 shrink-0">
          <div>
            <h2 className="text-lg font-bold text-text-primary tracking-tight">
              {initialValue ? "Edit Designation" : "New Designation"}
            </h2>
            <p className="text-xs text-text-muted mt-0.5">
              {initialValue ? "Update designation name" : "Add a new designation to the system"}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="px-6 pb-6 space-y-4">
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="e.g. Sr. Developer, Manager, Designer"
                value={name}
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 shrink-0 border-t border-border bg-background">
          ...
          >
            ...
            {initialValue ? "Update Designation" : "Create Designation"}
          </button>
        </div>
      </div>
    </div>

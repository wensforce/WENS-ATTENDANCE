...
// Quick presets shown in the date range dropdown
const DATE_PRESETS = [
  { key: "today", label: "Today", range: () => [todayStr(), todayStr()] },
  {
    key: "yesterday",
    label: "Yesterday",
    range: () => [shiftDays(-1), shiftDays(-1)],
  },
  { key: "last7", label: "Last 7 Days", range: () => [shiftDays(-6), todayStr()] },
  {
    key: "last30",
    label: "Last 30 Days",
    range: () => [shiftDays(-29), todayStr()],
  },
  {
    key: "thisMonth",
    label: "This Month",
    range: () => [startOfMonthStr(), todayStr()],
  },
...
const Attendance = () => {
  ...
  const [specialStartDate, setSpecialStartDate] = useState(todayStr());
  const [specialEndDate, setSpecialEndDate] = useState(todayStr());
  ...
  const [specialAttendanceModalOpen, setSpecialAttendanceModalOpen] =
    useState(false);
  const [editSpecialAttendance, setEditSpecialAttendance] = useState(null);
  const [specialAttendanceModalLoading, setSpecialAttendanceModalLoading] =
    useState(false);
  ...
  const debouncedSpecialSelectedEmployee = useDebounce(
  ...
  const fetchSpecialAttendance = async (page = 1) => {
    ...
    try {
      ...
      const response = await specialAttendanceApi.fetchAllSpecialAttendance(
        page,
        filters,
      );
  ...
  useEffect(() => {
    ...
    debouncedSpecialSelectedEmployee,
    specialStartDate,
    specialEndDate,
    activeTab,
  ]);
  ...
  const handleAttendanceFormSubmit = async (formData) => {
    ...
    try {
      // Leave is not an attendance status, it is stored as a single-day leave entry
      if (formData.isLeave) {
        await useHolidayApi.createHoliday({
          status: "LEAVE",
          subTypeId: formData.subTypeId,
          startDate: formData.startDate,
          endDate: formData.endDate,
          reason: formData.reason,
          employeeIds: [Number(formData.userId)],
        });
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message ||
          "Failed to save special attendance record",
      );
  ...
  const handleConfirmSpecialDelete = async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message ||
          "Failed to delete special attendance record",
      );
  ...
  const handleResetSpecialFilters = () => {
    ...
    setSpecialStartDate(todayStr());
    setSpecialEndDate(todayStr());
  };
  ...
  const todayRecords = attendance.filter(
    (a) => new Date(a.date).toDateString() === todayDateString,
  );
              ...
              <div className="px-6 py-4 border-b border-border">
                <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-4">
                  ...
                  <div className="sm:ml-auto flex items-center gap-2">
                    ...
                    {(search ||
                      selectedEmployee ||
                      !isTodayRange(startDate, endDate) ||
                      selectedStatus) && (
                      <button
                    ...
                    <div className="bg-background rounded-xl border border-border p-4 animate-in fade-in slide-in-from-top-2">
                      ...
                      <div className="flex flex-wrap gap-2 mb-4">
                        {DATE_PRESETS.map((preset) => {
                          const active =
                            matchPreset(startDate, endDate) === preset.key;
                          return (
                            <button
                              key={preset.key}
                              onClick={() => {
                              ...
                              className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${
                                active
                                  ? "bg-text-primary text-white border-text-primary"
                                  : "bg-surface border-border text-text-primary hover:border-text-primary"
                              }`}
                            >
                              {preset.label}
                            </button>
              ...
              <div className="px-6 py-4 border-b border-border">
                <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-4">
                  <div>
                    ...
                    <p className="text-xs text-text-secondary mt-0.5">
                      {specialFiltered.length} of {specialAttendance.length}{" "}
                      records
                    </p>
                  </div>
                  ...
                  <div className="sm:ml-auto flex items-center gap-2">
                    {(specialSearch ||
                      specialSelectedEmployee ||
                      !isTodayRange(specialStartDate, specialEndDate)) && (
                      <button
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setSpecialSelectedEmployee(e.target.value)
                      }
                      className="px-4 py-2 text-xs bg-background border border-border rounded-lg text-text-primary focus:outline-none focus:ring-1 focus:ring-text-primary transition"
                    >
                      <option value="">All Employees</option>
                    ...
                    <button
                      onClick={() =>
                        setSpecialDateFilterOpen(!specialDateFilterOpen)
                      }
                      className={`flex items-center justify-between px-4 py-2 text-xs font-medium rounded-lg border transition-all ${
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        <Calendar size={14} className="shrink-0" />
                        <span className="truncate">
                          {dateRangeLabel(specialStartDate, specialEndDate)}
                        </span>
                      </div>
                      <ChevronDown
                        size={14}
                  ...
                  {specialDateFilterOpen && (
                    <div className="bg-background rounded-xl border border-border p-4 animate-in fade-in slide-in-from-top-2">
                      ...
                      <div className="flex flex-wrap gap-2 mb-4">
                        {DATE_PRESETS.map((preset) => {
                          const active =
                            matchPreset(specialStartDate, specialEndDate) ===
                            preset.key;
                          return (
                            <button
                              key={preset.key}
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setSpecialStartDate(e.target.value)
                              }
                              className="flex-1 px-3 py-2 text-xs bg-surface border border-border rounded-lg text-text-primary focus:outline-none focus:ring-1 focus:ring-text-primary transition"
                            />
                          </div>
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setSpecialEndDate(e.target.value)
                              }
                              className="flex-1 px-3 py-2 text-xs bg-surface border border-border rounded-lg text-text-primary focus:outline-none focus:ring-1 focus:ring-text-primary transition"
                            />
                          </div>
              ...
              <div className="p-4 overflow-x-auto">
                <DataTable
                  ...
                  emptyText={
                    ...
                    specialEndDate
                      ? isTodayRange(specialStartDate, specialEndDate)
                        ? "No special attendance records for today"
